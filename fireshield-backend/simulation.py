...
# Minutes per time step - calibrated so 6 steps ≈ 30 min of a fast-moving fire
MINUTES_PER_STEP = 5.0


def compute_poi_threat_timeline(
    state_snapshots: list[np.ndarray],
    points_of_interest: list,
    bounds: dict,
) -> list[dict]:
    """
    For each POI, determine the earliest time step at which the fire reaches its grid cell.
    Returns a list of dicts with name, lat, lng, threat_step, minutes_to_reach, threat_level.
    """
    results = []
    for poi in points_of_interest:
        px = int(poi.x)
        py = int(poi.y)
        poi_lat = poi.lat if poi.lat else grid_to_latlng(px, py, bounds)[0]
        poi_lng = poi.lng if poi.lng else grid_to_latlng(px, py, bounds)[1]

        threat_step = None
        for step_idx, state in enumerate(state_snapshots):
            if 0 <= py < state.shape[0] and 0 <= px < state.shape[1]:
                if state[py, px] > 0:
                    threat_step = step_idx
                    break

        minutes = round(threat_step * MINUTES_PER_STEP, 1) if threat_step is not None else None

        if threat_step is None:
            threat_level = "Safe"
        elif threat_step <= 2:
        ...
        results.append({
            ...
            "lat": float(poi_lat),
            "lng": float(poi_lng),
            "threat_step": threat_step,
            "minutes_to_reach": minutes,
            "threat_level": threat_level,
        })

    # Sort: Critical first, then by minutes
    def sort_key(r):
        priority = {"Critical": 0, "High": 1, "Low": 2, "Safe": 3}
        return (priority.get(r["threat_level"], 4), r["minutes_to_reach"] or 9999)
...
def compute_evacuation_routes(
    ignition_x: int,
    ignition_y: int,
    poi_timeline: list[dict],
    bounds: dict,
) -> list[dict]:
    """
    For each threatened POI, generate a waypointed escape path that moves
    away from the ignition point toward the nearest grid edge.
    """
    routes = []
    ig_lat, ig_lng = grid_to_latlng(ignition_x, ignition_y, bounds)

    for poi in poi_timeline:
        if poi["threat_level"] == "Safe":
            continue

        poi_lat = poi["lat"]
        poi_lng = poi["lng"]

        # Direction vector AWAY from ignition
        dlat = poi_lat - ig_lat
        dlng = poi_lng - ig_lng
        dist = math.hypot(dlat, dlng)
        if dist < 1e-9:
            continue
        # Normalise and extend well beyond fire zone
        norm_lat = dlat / dist
        norm_lng = dlng / dist

        # Build waypoints: start at POI, then step away in 3 intermediate points
        path = [[poi_lat, poi_lng]]
        step_size = 0.012  # ~1.3 km per waypoint
        for i in range(1, 4):
            wlat = poi_lat + norm_lat * step_size * i
            wlng = poi_lng + norm_lng * step_size * i
            path.append([round(wlat, 6), round(wlng, 6)])

        routes.append({
            "poi_name": poi["name"],
            "urgency": poi["threat_level"].lower(),
            "path": path,
        })
