      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Generate a concise response plan from the latest model state.
          </p>
        </div>
        <button
      ...
      {!recommendation && !loading && (
        <div className="mt-4 grid place-items-center rounded-2xl border border-slate-200/10 bg-slate-950/55 px-4 py-7 text-center">
          ...
          <p className="mt-3 max-w-xs text-sm text-slate-400">
            Run the recommendation engine to produce evacuation, resource, and
            protection actions.
          </p>
        </div>
      )}
          ...
          <div className="rounded-2xl border border-slate-200/10 bg-slate-950/55 p-3">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {recommendation.road_closures.map((item) => (
                <span
                  className="rounded-full border border-slate-200/10 bg-slate-900/70 px-3 py-1 text-xs text-slate-200"
                  key={item}
                >
                  {item}
                </span>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              <div className="mb-1 flex justify-between text-xs text-slate-400">
                ...
                <span className="mono">
                  {recommendation.containment_estimate_percent}%
                </span>
              </div>
              <div className="h-2 rounded-full bg-slate-900">
                <div
