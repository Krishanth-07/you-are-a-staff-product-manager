...
function buildTerrainUrl(regionData) {
  ...
  for (let y = 0; y < regionData.grid_size; y += 1) {
    for (let x = 0; x < regionData.grid_size; x += 1) {
      ...
      const canopyMix = Math.max(0, Math.min(1, vegetation * 0.72 + (1 - elevation) * 0.28))
      const ridgeMix = Math.max(0, Math.min(1, elevation * 0.85 + vegetation * 0.15))
      const lowland = lerpColor([18, 62, 42], [31, 92, 58], canopyMix)
      const ridge = lerpColor([84, 110, 62], [132, 108, 76], ridgeMix)
      const gradient = context.createLinearGradient(x * cellSize, y * cellSize, (x + 1) * cellSize, (y + 1) * cellSize)
  ...
  for (let y = 0; y < regionData.grid_size; y += 1) {
    for (let x = 0; x < regionData.grid_size; x += 1) {
      ...
      const right = regionData.elevation_map[y][Math.min(regionData.grid_size - 1, x + 1)]
      const up = regionData.elevation_map[Math.max(0, y - 1)][x]
      const down = regionData.elevation_map[Math.min(regionData.grid_size - 1, y + 1)][x]
      const slope = ((right - left) + (down - up)) * 0.55
      const shade = Math.max(-0.18, Math.min(0.18, slope - (elevation - 0.5) * 0.04))
  ...
  for (let y = 0; y < regionData.grid_size; y += 1) {
    for (let x = 0; x < regionData.grid_size; x += 1) {
      ...
      context.fillStyle = noise > 0.93 ? 'rgba(255, 255, 255, 0.22)' : 'rgba(12, 74, 110, 0.16)'
      const size = cellSize * (0.12 + noise * 0.22)
      context.beginPath()
      context.arc(x * cellSize + cellSize * 0.55, y * cellSize + cellSize * 0.45, size, 0, Math.PI * 2)
      context.fill()
    }
  }
  ...
  for (let y = 0; y < regionData.grid_size; y += 1) {
    for (let x = 0; x < regionData.grid_size; x += 1) {
      ...
      if (status === 'burning') {
        ...
        const gradient = context.createRadialGradient(centerX, centerY, cellSize * 0.12, centerX, centerY, cellSize * 0.9)
        ...
        context.fillStyle = gradient
        context.fillRect(x * cellSize - cellSize * 0.2, y * cellSize - cellSize * 0.2, cellSize * 1.4, cellSize * 1.4)
...
function TerrainOverlay({ terrainUrl, fireUrl }) {
  ...
  return (
    <>
      ...
      {fireUrl && <ImageOverlay bounds={MAP_BOUNDS} opacity={0.92} url={fireUrl} />}
    </>
  )
}
...
function poiIcon(type) {
  ...
  const iconMarkup = renderToStaticMarkup(<style.Icon color="white" size={15} strokeWidth={2.25} />)
...
function ignitionIcon() {
  const iconMarkup = renderToStaticMarkup(<Navigation color="white" size={14} strokeWidth={2.5} />)
...
export default function FireMap({ externalParams, onSimulationUpdate, addLog }) {
  ...
  const currentCells = simulationData?.time_steps_data?.[Math.min(step, maxStep)] || []
  const affectedCells = currentCells.filter((cell) => cell.status === 'burning' || cell.status === 'burnt').length
  const ignitionPosition = [params.ignition_y + 0.5, params.ignition_x + 0.5]
  const terrainUrl = useMemo(() => (regionData ? buildTerrainUrl(regionData) : ''), [regionData])
  const fireUrl = useMemo(() => (regionData && simulationData ? buildFireUrl(regionData, simulationData, step) : ''), [regionData, simulationData, step])
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">Area affected: {affectedCells} cells (~{affectedCells * 4} ha)</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-3 text-xs text-slate-400">
              <span className="mono shrink-0 text-slate-100">Step {step + 1}/{maxStep + 1}</span>
              <input
        ...
        <div className="relative h-[66vh] min-h-125 overflow-hidden rounded-[28px] border border-white/8 bg-slate-950">
          ...
          {loading && <div className="absolute inset-0 z-10 grid place-items-center bg-slate-950/90 text-slate-300">Loading command map...</div>}
          {error && <div className="absolute inset-0 z-10 grid place-items-center bg-slate-950 text-red-300">{error}</div>}
          <MapContainer
            bounds={MAP_BOUNDS}
          ...
          >
            ...
            <Marker icon={ignitionIcon()} position={ignitionPosition}>
              <Popup>
                ...
                <br />
                X {params.ignition_x}, Y {params.ignition_y}
              </Popup>
            </Marker>
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-slate-400">
          <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-[#365c3d]" />Terrain</span>
          <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-[#f97316]" />Fire</span>
          <span className="flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-[#1a1210]" />Burnt</span>
        </div>
      </div>
    </section>
  )
